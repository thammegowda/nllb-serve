...
def parse_args():
    ...
    parser.add_argument(
        "-sl", "--src-lang", type=str, required=True, help="source language identifier; eg: eng_Latn"
    )
    parser.add_argument(
        "-tl", "--tgt-lang", type=str, required=True, help="Target language identifier; eg: eng_Latn"
    )
    parser.add_argument(
        '-i', '--inp', help='Input file', type=FileType('r', encoding='utf8'), default=sys.stdin
    )
    parser.add_argument(
        '-o', '--out', help='Output file', type=FileType('w', encoding='utf8'), default=sys.stdout
    )
    parser.add_argument(
        "-msl",
        "--max-src-chars",
        type=int,
        default=DEF_MAX_SRC_CHARS,
